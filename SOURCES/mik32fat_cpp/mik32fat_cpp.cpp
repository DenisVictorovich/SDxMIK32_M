...
MIK32FAT::FAT_Status_t MIK32FAT::Init(FAT_Descriptor_t* fs, MIK32_SPI_SD::SD_Descriptor_t* sd_card)
{
    // ToDo: сделать проверу томов на форматированность FAT32
    fs->card = sd_card;
    /* Read Master boot record */
    if (MIK32_SPI_SD::SD_SingleRead(fs->card, 0, fs->buffer) != 0) return FAT_DiskError;
    /* Read LBAs */
    uint8_t counter = 0, type_code, *ptr = fs->buffer + FAT_MBR_Partition0;
    ...
    /* Read LBA sector */
    if (MIK32_SPI_SD::SD_SingleRead(fs->card, fs->fs_begin, fs->buffer) != 0) return FAT_DiskError;
 ...
 * @return none
 */
void MIK32FAT::SetPointerToRoot(FAT_Descriptor_t* fs)
{
    fs->temp.cluster = 0;
}
 ...
 */
MIK32FAT::FAT_Status_t MIK32FAT::FindNextCluster(FAT_Descriptor_t* fs)
{
    /* Read FAT */
    uint32_t bias = (fs->temp.cluster / (512/4));
    if (MIK32_SPI_SD::SD_SingleRead(fs->card, fs->fat1_begin + bias, fs->buffer) != 0) return FAT_DiskError;
 ...
 */
MIK32FAT::FAT_Status_t MIK32FAT::FindByName(FAT_Descriptor_t* fs, char* name)
{
    char name_str[11];
    ...
    while (res == FAT_OK)
    {
        ...
        for (uint8_t sec=0; sec < fs->param.sec_per_clust; sec++)
        {
            ...
            {
                if (MIK32_SPI_SD::SD_SingleRead(fs->card, sector, fs->buffer) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
 ...
 * len - length of file. entire_in_dir_clust - number of entire in dir_sector. status - status of file/dir
 */
MIK32FAT::FAT_Status_t MIK32FAT::FindByPath(FAT_Descriptor_t* fs, char* path)
{
    /* calculate number of '/' symbols */
    uint8_t descend_number = 1, i = 0;
    while (path[i] != '\0')
    {
 ...
 * len - length of file. entire_in_dir_clust - number of entire in dir_sector. status - status of file/dir
 */
MIK32FAT::FAT_Status_t MIK32FAT::FindOrCreateByPath(FAT_Descriptor_t* fs, char* path)
{
    ...
    for (uint8_t k=0; k<descend_number; k++)
    {
        ...
        if (not_found)
        {
            res = Create(fs, ptr, k != (descend_number-1));
            if (res != FAT_OK) return res;
 ...
 */
MIK32FAT::FAT_Status_t MIK32FAT::FindFreeCluster(FAT_Descriptor_t* fs, uint32_t* new_cluster)
{
    ...
    do {
        x += 1;
        if (MIK32_SPI_SD::SD_SingleRead(fs->card, fs->fat1_begin + x, fs->buffer) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
 ...
 * - FAT_NoFreeSpace there are not free space on the partition
 */
MIK32FAT::FAT_Status_t MIK32FAT::TakeFreeCluster(FAT_Descriptor_t* fs, uint32_t cluster, uint32_t* new_cluster)
{
    ...
    *new_cluster = new_clus - 2;
    ...
    {
        if (MIK32_SPI_SD::SD_SingleRead(fs->card, fs->fat1_begin + (cluster / (512/4)), fs->buffer) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
        fs->prev_sector = fs->fat1_begin + (cluster / (512/4));
    }
    ptr = (uint32_t*)(fs->buffer + (cluster * fs->param.sec_per_clust) % 512);
    *ptr = new_clus;
    if (MIK32_SPI_SD::SD_SingleErase(fs->card, fs->fat1_begin + (cluster / (512/4))) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
    if (MIK32_SPI_SD::SD_SingleWrite(fs->card, fs->fat1_begin + (cluster / (512/4)), fs->buffer) != 0) return FAT_DiskError;
    if (MIK32_SPI_SD::SD_SingleErase(fs->card, fs->fat2_begin + (cluster / (512/4))) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
    if (MIK32_SPI_SD::SD_SingleWrite(fs->card, fs->fat2_begin + (cluster / (512/4)), fs->buffer) != 0) return FAT_DiskError;
    /* Find sector of FAT containing previous cluster link */
    if (fs->fat1_begin + (new_clus / (512/4)) != fs->prev_sector)
    {
        if (MIK32_SPI_SD::SD_SingleRead(fs->card, fs->fat1_begin + (new_clus / (512/4)), fs->buffer) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
        fs->prev_sector = fs->fat1_begin + (new_clus / (512/4));
    }
    ptr = (uint32_t*)(fs->buffer + link);
    *ptr = 0x0FFFFFFF;
    if (MIK32_SPI_SD::SD_SingleErase(fs->card, fs->fat1_begin + (new_clus / (512/4))) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
    if (MIK32_SPI_SD::SD_SingleWrite(fs->card, fs->fat1_begin + (new_clus / (512/4)), fs->buffer) != 0) return FAT_DiskError;
    if (MIK32_SPI_SD::SD_SingleErase(fs->card, fs->fat2_begin + (new_clus / (512/4))) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
    if (MIK32_SPI_SD::SD_SingleWrite(fs->card, fs->fat2_begin + (new_clus / (512/4)), fs->buffer) != 0) return FAT_DiskError;
    return FAT_OK;
}

MIK32FAT::FAT_Status_t MIK32FAT::FileOpen(FAT_File_t* file, FAT_Descriptor_t* fs, char* path, char modificator)
{
    file->fs = fs;
    ...
    {
        case 'R':
            SetPointerToRoot(file->fs);
            res = FindByPath(file->fs, path);
            if (res != FAT_OK) return res;
            /* Access settings */
        ...
        case 'W':
            SetPointerToRoot(file->fs);
            res = FindOrCreateByPath(file->fs, path);
            if (res != FAT_OK) return res;
            /* Access settings */
            file->dir_sector = file->fs->temp.dir_sector;
            file->entire_in_dir_clust = file->fs->temp.entire_in_dir_clust;
 ...
 * @returns
 */
MIK32FAT::FAT_Status_t MIK32FAT::FileClose(FAT_File_t* file)
{
    if ((file->modificator == 'W') || (file->modificator == 'A'))
    {
        ...
        if (file->writing_not_finished == true)
        {
            ...
            if (MIK32_SPI_SD::SD_SingleErase(file->fs->card, sector) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
            if (MIK32_SPI_SD::SD_SingleWrite(file->fs->card, sector, file->fs->buffer) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
        }
        /* Write new file length */
        sector = file->fs->data_region_begin + file->dir_sector;
        if (MIK32_SPI_SD::SD_SingleRead(file->fs->card, sector, file->fs->buffer) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
        file->fs->prev_sector = sector;
        uint32_t* ptr = (uint32_t*)&file->fs->buffer[file->entire_in_dir_clust + FAT_DIR_FileSize];
        *ptr = file->len;
        if (MIK32_SPI_SD::SD_SingleErase(file->fs->card, sector) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
        if (MIK32_SPI_SD::SD_SingleWrite(file->fs->card, sector, file->fs->buffer) != 0) return FAT_DiskError;
    }
    return FAT_OK;
}
 ...
 */
uint32_t MIK32FAT::ReadFile(FAT_File_t* file, char* buf, uint32_t quan)
{
    if (file->modificator != 'R') return 0;
    uint32_t counter = 0, start_addr = (file->addr - file->addr % (512 * file->fs->param.sec_per_clust));
    ...
    while ((quan > 0) && (file->len > 0))
    {
        ...
        {
            if (MIK32_SPI_SD::SD_SingleRead(file->fs->card, sector, file->fs->buffer) != 0) return counter;
            file->fs->prev_sector = sector;
        }
        /* Reading sector */
 ...
 */
uint32_t MIK32FAT::WriteFile(FAT_File_t* file, const char* buf, uint32_t quan)
{
    ...
    while (quan > counter)
    {
        ...
        if ((buf_idx != 0) && (file->writing_not_finished == false))
        {
            ...
            if (MIK32_SPI_SD::SD_SingleRead(file->fs->card, sector, file->fs->buffer) != MIK32_SPI_SD::SD_OK) return counter;
            file->fs->prev_sector = sector;
        }
        ...
        if ((file->addr % file->fs->param.clust_len == 0) && (file->addr != 0))
        {
            // Make new cluster
            uint32_t value_buf;
            FAT_Status_t res = TakeFreeCluster(file->fs, file->cluster, &value_buf);
        ...
        if (buf_idx >= 512)
        {
            ...
            if (MIK32_SPI_SD::SD_SingleErase(file->fs->card, sector) != MIK32_SPI_SD::SD_OK) return counter;
            if (MIK32_SPI_SD::SD_SingleWrite(file->fs->card, sector, file->fs->buffer) != MIK32_SPI_SD::SD_OK) return counter;
            //
            buf_idx = 0;
        }
    }
 ...
 * @returns
 */
MIK32FAT::FAT_Status_t MIK32FAT::Create(FAT_Descriptor_t* fs, char* name, bool dir)
{
    /* Find free cluster in FAT */
    uint32_t new_cluster, *ptr;
    int32_t x = -1, link;
    do {
        x += 1;
        if (MIK32_SPI_SD::SD_SingleRead(fs->card, fs->fat1_begin + x, fs->buffer) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
    ...
    new_cluster = (x * 128 + (link>>2));
    *ptr = 0x0FFFFFFF;
    if (MIK32_SPI_SD::SD_SingleErase(fs->card, fs->fat1_begin + x) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
    if (MIK32_SPI_SD::SD_SingleWrite(fs->card, fs->fat1_begin + x, fs->buffer) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
    if (MIK32_SPI_SD::SD_SingleErase(fs->card, fs->fat2_begin + x) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
    if (MIK32_SPI_SD::SD_SingleWrite(fs->card, fs->fat2_begin + x, fs->buffer) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
    ...
    if (dir == true)
    {
        ...
        ent[1].FstClusHI = 0;
        /* Write data */
        if (MIK32_SPI_SD::SD_SingleErase(fs->card, fs->data_region_begin+(new_cluster-2)*fs->param.sec_per_clust) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
        if (MIK32_SPI_SD::SD_SingleWrite(fs->card, fs->data_region_begin+(new_cluster-2)*fs->param.sec_per_clust, fs->buffer) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
    }
    ...
    {
        uint32_t value;
        res = TakeFreeCluster(fs, fs->temp.cluster, &value);
        if (res != FAT_OK) return res;
        entire = 0;
    ...
    if (MIK32_SPI_SD::SD_SingleErase(fs->card, sector) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
    if (MIK32_SPI_SD::SD_SingleWrite(fs->card, sector, fs->buffer) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
    return FAT_OK;
}

MIK32FAT::FAT_Status_t MIK32FAT::DeleteTemp(FAT_Descriptor_t* fs)
{
    uint32_t sector;
    ...
    do {
        if (sector != fs->fat1_begin + (cluster / (512/4)))
        {
            ...
            if (dummy != 0)
            {
                if (MIK32_SPI_SD::SD_SingleErase(fs->card, dummy) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
                if (MIK32_SPI_SD::SD_SingleWrite(fs->card, dummy, fs->buffer) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
            }
            if (MIK32_SPI_SD::SD_SingleRead(fs->card, sector, fs->buffer) != MIK32_SPI_SD::SD_OK) return FAT_DiskError;
            fs->prev_sector = sector;
        }
 ...
 * @returns
 */
MIK32FAT::FAT_Status_t MIK32FAT::Delete(FAT_Descriptor_t* fs, char* path)
{
    FAT_Status_t res;
    res = FindByPath(fs, path);
    if (res != FAT_OK) return res;
    return DeleteTemp(fs);
}
